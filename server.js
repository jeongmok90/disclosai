...
[1문장 — 실적 평가, 필수]
①②에 [실제 데이터]가 있으면 무조건 첫 문장에 구체적 수치를 넣어 작성하세요.
형식 예시: "매출 10.3B달러(컨센서스 +3.6% 상회), EPS 1.37달러(+6.2% 상회)로 어닝 서프라이즈를 기록했으나..."
[실제 데이터]가 없으면 원문에서 직접 추출한 수치로 대체하세요.

[2~3문장 — 리스크/기회 판단, 조건부]
아래 조건에 해당하는 항목만 추가하세요. 해당 없으면 생략:
- 공시 전 모멘텀 ⚠️: 급등 선반영 → 차익 실현 압력 언급
- 밸류에이션 ⚠️ 극단적 고평가: 실망 매물 위험 언급
- 성장 추이: 실적이 컨센서스를 상회했다면 "성장 둔화" 표현 사용 금지. 컨센서스 miss일 때만 둔화 언급
${source === 'dart'
  ? `- 환율 ⚠️: 수출 비중 높은 기업(반도체·자동차·조선)은 강달러 수혜, 수입 비중 높은 기업은 비용 부담`
  : `- 매크로: VIX > 25이거나 금리 > 4.5% 또는 < 3.5%일 때만\n- 내부자 거래: 매도 금액 $1M 이상일 때만`}

절대 금지: 수치 없이 "높은 수준", "급등", "둔화" 같은 단어만 나열하지 마세요. 반드시 구체적 숫자와 함께 쓰세요.
